...
public class CosProbModel 
{
	...
	public CosProbModel(rses.spatial.GISLayer geography, RSSObservation[] observations, 
			...
			Distribution speeddist, Map<String, PMF> attenuation, double noise,
			boolean quickMode) 
	throws java.io.IOException
	{
		...
		this.solve(pathlengths, pathsteps, pathdistances, pathtrees, speeddist, quickMode);
	}
	...
	throws java.io.IOException
	{
		...
		PMF[][] pmfs = getPMFs(geography, observations, bestP0, spatialPrior, true, pathdistances);
		...
		PMF[] sentinels = pmfs[0]; 
		
		writePath("ML_path_raw.kml", finalpmfs, observations[0].time);		

		

		//if we are not in quick mode, we build the massive djikstra graph and
		//find the maximum likelihood path from that (we overwrite the 'quick'
		//solution that we have worked out above)
		if(!quickMode)
		{
			...
			Distribution speeddist) throws java.io.IOException
	{
		...
		PMF[][] pmfsAndsentinels = getPMFs(geography, observations, P0, spatialPrior, false, pathdistances);
