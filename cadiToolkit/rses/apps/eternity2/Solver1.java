...
public class Solver1 
{
		...
		while(true)
		{
			...
			for(int j = 0; j < pieces.length; j++)
				if(!isplaced[j]) 
					piecesleft = true;
			if(!piecesleft)
				System.exit(0); //no pieces left to place
			...
			{
				int[] xy = E2Util.calcXYfromSquareNum(unplaced, j);
			...
			if(pause) {
				...
				if(line.trim().length() != 0) {
					...
					int[] bxy = E2Util.calcXYfromSquareNum(b, bi);
	...
	public static boolean doMove(Board b, Piece[] pieces, boolean[] isplaced)
	{
		...
		if(boardindex == -1)
		{
			...
			{
				int[] xy = E2Util.calcXYfromSquareNum(b, i);
			...
			//just choose the move with the highest probability
			for(int i = 0; i < matrix.length; i++)
			{
				...
				//if the following code in uncommented, then
				//we force the moved piece to be adjacent
				//to an already moved piece or an edge
				
				if(xy[0] > 0 && xy[0] < b.board.length-1 &&
				   xy[1] > 0 && xy[1] < b.board[0].length-1)
				{
					if(b.board[xy[0]-1][xy[1]] == null &&
					   b.board[xy[0]+1][xy[1]] == null &&
				       b.board[xy[0]][xy[1]-1] == null &&
					   b.board[xy[0]][xy[1]+1] == null)
						continue;
				}
				...
				for(int j = 0; j < matrix[0].length; j++)
				{
					if(isplaced[j])
						continue;
					for(int r = 0; r < 4; r++)
						if(matrix[i][j][r] > maxprob) {
							maxprob = matrix[i][j][r];
							boardindex = i;
							pieceindex = j;
						}
				}
			...
			/*
			//choose the board square to move on by calculating
			//the board square with the minimum entropy
			double minentropy = Double.MAX_VALUE;
			for(int i = 0; i < matrix.length; i++)
			{
				//first of all, we make sure the square is empty,
				//because clearly those squares arent in consideration
				int[] xy = calcXYfromSquareNum(b, i);
				if(b.board[xy[0]][xy[1]] != null)
					continue;
			
				double d = boardentropy(matrix[i]);
				if(d < minentropy) {
					minentropy = d;
					boardindex = i;
				}
			...
			//ok, we know the board square with the minimum
			//entropy.
			//Find the most likely piece for that square.
			double maxprob = Double.NEGATIVE_INFINITY;
			for(int i = 0; i < matrix[0].length; i++)
			{
				//the sum over all rotations
				double psum = Util.getSum(matrix[boardindex][i]); 
			...
			//if(maxprob <= 0.0)
			//	throw new IllegalStateException("Best move has probability zero! Should be impossible, as we should pick this up earlier than here.");
		}
		...
		//now place it
		int[] xy = E2Util.calcXYfromSquareNum(b, boardindex);
			...
			Piece[] pieces, boolean[] isplaced)
	{
		...
		for(int i = 0; i < normalizeiterations; i++)
		{
			...
			matrix = normalizeByColumns(matrix, isplaced);
		}
		matrix = normalizeByRows(matrix);
			...
			Piece[] pieces, int toplace, double[][][] probs)
	{
		...
		else if(x > 0) 
		{
			...
			{
				int tryindex = E2Util.calcLinearizedBoardIndex(board, x-1, y);
		...
		else if(y > 0) 
		{
			...
			{
				int tryindex = E2Util.calcLinearizedBoardIndex(board, x, y-1);
				//try all pieces in neighbouring squares
				for(int i = 0; i < pieces.length; i++)
				{
					if(i == toplace) continue;
			...
			boolean[] isplaced)
	{
		for(int i =0; i < matrix[0].length; i++) //loop over pieces
		{
			...
			for(int j = 0; j < matrix.length; j++) //loop over squares
			{
				...
				if(hasnonzero)
					numnonzero++;
				if(hasnonzero && !isplaced[i])
					nonzeroindex = j;
			}
			
			if(numnonzero == 1 && !isplaced[i]) //only 1 square possible for this piece
				throw new ForcedMoveException(nonzeroindex, i, matrix);
